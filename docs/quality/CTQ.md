...
| CTQ-002 | El usuario espera que sus respuestas no se pierdan | Persistencia de datos en base de datos | % de respuestas guardadas correctamente | 100% de datos guardados sin duplicados ni pérdidas | Prueba de guardado en PostgreSQL (test/db.test.js) | Must | #2 |
...
| CTQ-004 | El usuario necesita que sus respuestas estén protegidas de accesos no autorizados | Protección de datos de los encuestados | % de accesos no autorizados bloqueados | 100% de intentos rechazados | Login por sesión (server/auth.js) + prueba de acceso restringido (test/auth.test.js) | Must | #4 |
